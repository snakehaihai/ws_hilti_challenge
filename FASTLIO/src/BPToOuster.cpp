...
public:
    ...
    {
        hesaiCloudSub = nh_ptr->subscribe<sensor_msgs::PointCloud2>("/rslidar_points", 50, &HesaiToOuster::cloudHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr &msgIn)
    {
        ...
        for (size_t i = 0; i < cloudsize; i++)
        {
            ...
            dst.ring = src.ring;
        //     std::cout<<"laserCloudHesai.points[0].timestamp  "<< laserCloudHesai.points[0].timestamp  <<std::setprecision(16) <<std::endl;
        //       std::cout<<"msgIn->header.stamp.toSec()  "<< msgIn->header.stamp.toSec()  <<std::setprecision(16) <<std::endl;
        //   std::cout<<"src.timestamp  "<< src.timestamp  <<std::setprecision(16) <<std::endl;
          
            // std::cout<<"msgIn-T "<< msgIn->header.stamp.toSec() <<std::setprecision(16) << std::endl;
        //    dst.t = (int)((src.timestamp - msgIn->header.stamp.toSec()) * 1e9f);
            dst.t = (int)((src.timestamp -  laserCloudHesai.points[0].timestamp  ) * 1e9f);
            // dst.t = (int)((src.timestamp  ));
